...
<template>
  <div class="p-3.5 sm:p-6 md:p-8 space-y-4 sm:space-y-6 max-w-[1400px] mx-auto select-none">
    ...
    <header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
      <div>
        <h1 class="text-xl sm:text-2xl font-extrabold text-slate-800 tracking-tight">Follow Up Client</h1>
        <p class="text-[11px] sm:text-xs text-slate-500 mt-0.5 sm:mt-1 font-medium">Kelola client yang perlu dihubungi kembali.</p>
      </div>
    </header>

    <!-- Toolbar: Tabs on Left, Search on Right -->
    <div class="bg-white rounded-2xl p-3 sm:p-4 shadow-xs border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
      <!-- Tabs with no-scrollbar -->
      <div class="flex items-center gap-1.5 overflow-x-auto no-scrollbar text-xs font-semibold pb-1 sm:pb-0">
        <button 
          @click="activeTab = 'all'"
          class="px-3 py-2 sm:px-3.5 rounded-xl transition-all cursor-pointer whitespace-nowrap"
      ...
      <div class="relative w-full sm:w-64">
        ...
        <input 
          ...
          class="w-full pl-10 pr-3.5 py-2 sm:py-2.5 rounded-xl border border-slate-200 text-xs font-medium text-slate-700 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all"
        />
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-5">
      <div 
        ...
        :key="item.id"
        class="bg-white rounded-2xl p-4 sm:p-5 shadow-xs border border-slate-100 flex flex-col justify-between hover:shadow-md transition-all duration-200"
      >
        <div>
          <!-- Top Row: Avatar, Name, Business -->
          <div class="flex items-center gap-3 mb-3 sm:mb-4">
            <div class="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-slate-100 text-slate-700 flex items-center justify-center font-bold text-sm border border-slate-200/80 shrink-0">
              {{ item.name.charAt(0) }}
            </div>
            <div>
          ...
          <div class="space-y-1.5 py-2.5 sm:py-3 border-t border-slate-100 text-xs">
        ...
        <!-- Action Buttons: WhatsApp (Green) + Detail (White) -->
        <div class="flex items-center gap-2 pt-3 border-t border-slate-100">
          <button 
            @click="sendWhatsApp(item)"
            class="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition-all shadow-xs cursor-pointer active:scale-95"
          >
          ...
          <button 
            ...
            class="px-4 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 text-xs font-semibold transition-all cursor-pointer active:scale-95"
          >
            <span>Detail</span>
          </button>
        </div>
    ...
    <!-- Toast Notification Alert (Positioned above mobile bottom bar) -->
    <div 
      v-if="toastMessage"
